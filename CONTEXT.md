...
* Dashboard junction: the meeting surface where the attached dashboard card's top edge joins the bar's bottom edge. Shaped as a concave fillet, material added in the reflex corner, so bar and card read as one surface.
* Junction radius: the fillet radius of the dashboard junction. Lives on `DashboardService.junctionRadius`, defaults to 16, user adjustable 0 to 32, where 0 renders the square join. The card window widens to `dashboardWidth + 2 × junction radius`; the card body stays `dashboardWidth`.
